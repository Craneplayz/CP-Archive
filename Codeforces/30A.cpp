...
void solve(){
    ...
    elif(A==0) cout<<"No solution\n";
    elif(d<0){
        if(fmod(n,2)){
            double C=pow(abs(d),1/n);
            if(abs(floor(C)-C)<0.000001||abs(C-ceil(C))<0.000001) cout<<-1*C<<'\n';
            else cout<<"No solution\n";  
        }
        else cout<<"No solution\n";
   }
    else{
        double C=pow(B/A,1/n);
        if(abs(C-floor(C))<0.000001||abs(ceil(C)-C)<0.000001) cout<<C<<'\n';
        else cout<<"No solution\n";
    }
}
